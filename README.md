## Objective:
Build a decentralized Supply Chain Management System using Hyperledger Fabric 2.4.9. 
The system will involve three organizations (Producer, Supplier, and Wholeseller), each with 
two peers. The project requires setting up 3 Raft ordering nodes, creating three channels, 
writing a smart contract for the complete product journey, and developing Fabric Gateway 
APIs accessible via Postman.
## Problem Statement:
You are tasked with creating a Supply Chain Management System that involves three 
organizations: Producer, Supplier, and Wholeseller. Each organization will participate in a 
permissioned blockchain network with two peers, three Raft-based ordering nodes, and three 
channels to manage the interactions between organizations. The goal is to track the complete 
product journey across the supply chain using smart contracts, and expose relevant actions 
through APIs.
...
```
## Channel Creation and Chaincode
```bash
export PATH=${PWD}/../bin:${PWD}:$PATH
