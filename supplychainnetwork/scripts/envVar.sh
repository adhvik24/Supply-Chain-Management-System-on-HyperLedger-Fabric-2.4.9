...
setGlobals() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    ...
    export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/wholeseller.example.com/users/Admin@wholeseller.example.com/msp
    export CORE_PEER_ADDRESS=localhost:6056
...
setGlobalsCLI() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    ...
    export CORE_PEER_ADDRESS=peer0.wholeseller.example.com:6051
  elif [ $USING_ORG -eq 4 ]; then
    export CORE_PEER_ADDRESS=peer1.producer.example.com:7056
  elif [ $USING_ORG -eq 5 ]; then
    export CORE_PEER_ADDRESS=peer1.supplier.example.com:9056
  elif [ $USING_ORG -eq 6 ]; then
    export CORE_PEER_ADDRESS=peer1.wholeseller.example.com:6056
  else
    errorln "ORG Unknown"
  fi
}
